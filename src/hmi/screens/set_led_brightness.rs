...
pub struct SetLedBrightnessScreen {
    led_brightness: u8,
}

impl SetLedBrightnessScreen {
    pub fn new() -> Self {
        Self { led_brightness: 0 } // TODO need to retrieve this from the settings
    }

    fn process_app_data(&mut self, data: ApplicationData) {
        if let ApplicationData::LedBrightness(x) = data {
            self.led_brightness = x
        }
    }
}

impl UiInputHandler for SetLedBrightnessScreen {
    fn ui_input_handler(
        &mut self,
        input: UiInput,
        ...
        match input {
            ...
            UiInput::EncoderClockwise => {
                ...
                ui_channel_publisher.publish_immediate(
                    UiActionsMessage::LedBrightnessChangeRequest(self.led_brightness),
                );
            }
...
impl UiDrawer for SetLedBrightnessScreen {
        ...
        DI: DisplayInterface,
    {
        ...
        Text::with_alignment(
            count_string.as_str(),
            Point::new(
                (display.size().width / 2) as i32,
                (display.size().height / 2) as i32,
            ),
            text_style,
            Alignment::Center,
        )
