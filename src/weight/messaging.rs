...
pub type WeightChannelSubscriber<'a> = Subscriber<
    ...
    CHANNEL_PUBS,
>;
pub type WeightChannelPublisher<'a> =
    Publisher<'a, CriticalSectionRawMutex, WeightEvents, CHANNEL_DEPTH, CHANNEL_SUBS, CHANNEL_PUBS>;
    ...
    async fn get_result(&mut self) -> Result<(), WeighingError> {
        loop {
            match self.weight_event_rx.next_message_pure().await {
                WeightEvents::RequestCompleted(_) => {
                    return Ok(());
                }
                WeightEvents::RequestFailed(e) => {
                    return Err(e);
                }
                _ => {}
            }
