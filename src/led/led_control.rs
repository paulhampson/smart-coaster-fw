...
pub enum LedArrayMode {
    ...
    SingleColourWheel {  colour: RGB8, speed: f32, repetitions: f32 },
    Pulse{ colour: RGB8, speed: f32 },
    StaticColour{ colour: RGB8 },
}
    ...
    P: Instance,
{
    fn set_mode(&mut self, mode: LedArrayMode) {
        ...
        match self.array_mode {
            ...
            LedArrayMode::SingleColourWheel{colour, speed, repetitions} => { self.set_speed_factor(speed); self.set_repetition_factor(repetitions); self.base_colour = colour; },
            LedArrayMode::Pulse{colour, speed} => {self.set_speed_factor(speed); self.base_colour = colour; },
            LedArrayMode::StaticColour{ colour } => { self.base_colour = colour; },
    ...
    async fn led_update(&mut self) {
        match self.array_mode {
            ...
            LedArrayMode::SingleColourWheel{ colour:_, speed:_, repetitions:_ } => {self.single_colour_wheel()}
            LedArrayMode::Pulse{ colour:_, speed:_, }  => {self.pulse()}
    ...
    P: Instance,
{
    ...
    fn pulse(&mut self) {
        ...
        let current_colour:RGB8 = ((brightness_factor * self.base_colour.r as f32).round() as u8,
                                   (brightness_factor * self.base_colour.g as f32).round() as u8,
                                    (brightness_factor * self.base_colour.b as f32).round() as u8 ).into();
